  ...
  return (
    ...
    >
      <div className="text-center mb-10">
        <Motion className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
          Mục lục
        </Motion>
        <Motion className="w-16 h-1 bg-blue-600 mx-auto rounded-full" delay={0.2} children={undefined}></Motion>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tableOfContents.map((item, index) => (
          <MotionHover scale={1.03} tapScale={0.97} key={item.id}>
            <Motion delay={0.2}>
              <div
                key={item.id}
